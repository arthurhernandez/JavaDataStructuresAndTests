...
public class SeamCarvingTest {
    ...
    public void getHVneighbors () {
        ...
        ns = sc.getHVneighbors(height-2, width - 2);
        assertFalse(ns.contains(new Position(height-2,0)));
        assertFalse(ns.contains(new Position(height-1, 1)));
        assertEquals(4, ns.size());

        ns = sc.getHVneighbors(height-(height / 2),width - (width / 2));
        assertFalse(ns.contains(new Position(height-2,width/2)));
        assertFalse(ns.contains(new Position(height-1, width/2-1)));
        assertFalse(ns.contains(new Position(height-1, width/2+1)));
        assertEquals(4, ns.size());

        ns = sc.getHVneighbors(height/ 3, width / 3);
        assertFalse(ns.contains(new Position(height-2,width-1)));
        assertFalse(ns.contains(new Position(height-2, width-1)));
        assertEquals(4, ns.size());
        ...
        ns = sc.getHVneighbors(height - (height - 1),width/2);
        assertFalse(ns.contains(new Position(height-2,width/2)));
        assertFalse(ns.contains(new Position(height-1, width/2-1)));
        assertFalse(ns.contains(new Position(height-1, width/2+1)));
        assertEquals(4, ns.size());

        ns = sc.getHVneighbors(height-1,width - (width - 1));
        assertFalse(ns.contains(new Position(height-2,width-1)));
        assertFalse(ns.contains(new Position(height-2, width-1)));
        assertEquals(3, ns.size());
    ...
    public void computeEnergy () {
        ...
        //tests
        assertEquals(6, sc.computeEnergy(100,53));
        assertEquals(0, sc.computeEnergy(125,53));
        assertEquals(0, sc.computeEnergy(0,0));
        assertEquals(19, sc.computeEnergy(15,15));
        assertEquals(150, sc.computeEnergy(200,25));
        assertEquals(0, sc.computeEnergy(10,10));
        assertEquals(0, sc.computeEnergy(height - 1,width -1 ));
        assertEquals(26, sc.computeEnergy(599,359));
        assertEquals(23, sc.computeEnergy(629,4));
        assertEquals(60, sc.computeEnergy(75,75));
    }
    ...
    public void seamBig () {
        ...
        try {
            ...
            assertEquals(new Position(10, 55), seam.getFirst());
            seam = seam.getRest();
        }
        catch (EmptyListE e) {}
    }
